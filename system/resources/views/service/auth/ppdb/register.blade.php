...
<div class="w3-display-middle">
	<div class="w3-row">
		<div class="w3-col s12 m12 l12 w3-container w3-card w3-light-grey w3-round-small boxContainer">
			...
			<form method="POST" action="{{route('service.auth.ppdb.signin')}}">
				@csrf
				...
				<div class="padding-top-bottom-16">
					...
					">
						...
						<label id="showPassword" style="cursor:pointer"><i class="fas fa-eye"></i></label>
					</div>
				</div>
				<div class="w3-padding-16">
					<button class="w3-block w3-button w3-blue w3-hover-indigo">{{trans('ppdb.hints.signin')}}</button>
				</div>
			</form>
			<form method="POST" action="{{route('service.auth.ppdb.register')}}">
				@csrf
				@if ($errors->any())
...
$(document).ready(function(){
	$('#showPassword').click(function(){
		...
		$(this).prev().prop('type', type=='text'? 'password' : 'text');
	});
});
</script>
@endSection