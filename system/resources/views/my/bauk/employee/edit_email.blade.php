<!--<div class="w3-row">-->
	...
	">
		<div class="input-group
			...
			<input name="email_id[{{$index}}]" type="hidden" value="{{old('email_id.'.$index, isset($email->id)? $email->id : '')}}" />
			<input name="email[{{$index}}]" type="text" 
				value="{{old('email.'.$index, isset($email->email)? $email->email : '')}}"
