...
class MnjKaryawanController extends Controller {
	...
	public function export(Request $req){
		$format = strtoupper($req->input('format'));
		return \App\DBModels\JNBauk\Karyawan::export($format, date('dmY',time()).'DaftarKaryawan');
	}
	...
	public function getTableDataKarayawan(Request $request){
		...
		if ($query){
			foreach(explode("&", $query) as $qer){
				$keyValue = explode("=", $qer);
				$queryData->where($keyValue[0],"=",$keyValue[1]);
			}
		}
		
		$queryTotal = $queryData->count();
