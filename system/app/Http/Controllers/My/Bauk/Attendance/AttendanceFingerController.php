...
use App\Http\Requests\My\Bauk\Attendance\Fingers\PostRequest as FingerPostRequest;
...
class AttendanceFingerController extends Controller
{
    public function show($nip=false, $year=false, $month=false, $day=false){
		...
		return view('my.bauk.attendance.fingers.landing',[
			'date'=> $date,
			'employee'=> $employee,
			'attendance'=> $employee->attendances()->where('date', '=', $formattedDate)->first(),
	...
	public function post(FingerPostRequest $req){
		...
		$attendanceData = $attendanceData?: new \App\Libraries\Bauk\EmployeeAttendance;
